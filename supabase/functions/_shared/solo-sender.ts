...
export function cleanName(mediaUrl: string): string {
  ...
  return decodeURIComponent(
    rawFileName.includes("_")
      ? rawFileName.split("_").slice(1).join("_")
      : rawFileName,
  );
}
...
export interface SendViaSoloArgs {
  // O sender não consulta o banco; o campo é mantido para compatibilidade com
  // os consumidores existentes que passam o cliente service-role.
  supabase: unknown;
  /**
 ...
 * Timeout: 15 seconds.
 */
export async function sendViaSolo(
  args: SendViaSoloArgs,
): Promise<SendViaSoloResult> {
  const {
    supabase,
    equipeId,
    instanceName,
  ...
  if (!baseUrl || !apiKey) {
    return {
      ok: false,
      error: "Missing WHATSMIAU_BASE_URL or WHATSMIAU_API_KEY env vars",
    };
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        ok: false,
        error: `Solo API returned ${response.status}: ${
    ...
    console.log(
      "[SoloSender] sent successfully | instance:",
      instanceName,
      "| providerMessageId:",
      providerMessageId,
    );
