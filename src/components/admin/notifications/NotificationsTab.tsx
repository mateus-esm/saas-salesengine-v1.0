...
import { Loader2, MessageSquare, Mail, Bell, Send, KeyRound } from "lucide-react";
...
function TemplatesPanel() {
  ...
  const save = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("admin-notifications", {
        body: { action: "save_template", type: editing.type, title, body },
      });
      if (error) throw error;
      if (data?.error) throw new Error(data.message ?? data.error);
      toast({ title: title || body ? "Mensagem salva" : "Template removido" });
      setEditing(null);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Quando um tipo tem mensagem escrita aqui, ela substitui o texto que o sistema mandaria.
          Sem mensagem, vale o texto padrão embutido no código. Use <code>{"{{variavel}}"}</code> para
          encaixar dados — variável que não existir some do texto, o cliente nunca vê chave solta.
        </CardDescription>
      </CardHeader>
      ...
      <Dialog open={editing !== null} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            <DialogTitle>{editing?.description ?? editing?.type}</DialogTitle>
            <DialogDescription>
              Deixe os dois campos vazios para voltar ao texto padrão do sistema.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            ...
            {!!editing?.variables?.length && (
              <div className="rounded-lg bg-muted/50 p-3">
                <p className="text-xs font-medium mb-1.5">Variáveis disponíveis neste tipo</p>
